...
class GoTest(unittest.TestCase):
    ...
    def test_to_indicator_actions_pass(self):
        ...
        self.assertTrue(jnp.all(lax.eq(indicator_moves, jnp.zeros_like(indicator_moves))))
    ...
    def test_to_indicator_actions_pass_and_move(self):
        ...
        self.assertTrue(jnp.all(lax.eq(indicator_moves[0], jnp.zeros_like(indicator_moves[0]))))
        self.assertEqual(jnp.all(lax.eq(indicator_moves[1], jnp.array([[[[True, False],
                                                                         [False, False]],
                                                                        [[False, False],
                                                                         [False, False]],
                                                                        [[False, False],
    ...
    def test_pass_sets_pass_layer(self):
        state = go.new_states(2)
        state = go.next_states(state, go.to_indicator_actions([None], state))
        self.assertTrue(jnp.all(lax.eq(state[4]), jnp.ones_like(state[4])))

    def test_two_consecutive_passes_ends_game(self):
        state = go.new_states(2)
        self.assertTrue(jnp.all(lax.eq(state[5]), jnp.zeros_like(state[5])))
        state = go.next_states(state, go.to_indicator_actions([None], state))
        self.assertTrue(jnp.all(lax.eq(state[5]), jnp.zeros_like(state[5])))
        state = go.next_states(state, go.to_indicator_actions([None], state))
        self.assertTrue(jnp.all(lax.eq(state[5]), jnp.ones_like(state[5])))
