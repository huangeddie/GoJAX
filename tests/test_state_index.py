...
class StateIndexTestCase(chex.TestCase):
    @parameterized.named_parameters({
        'testcase_name': 'pass', 'indicator_actions': [[[False, False], [False, False]]], 'expected_indices': 4
    }, {
        'testcase_name': 'first_index', 'indicator_actions': [[[True, False], [False, False]]], 'expected_indices': 0
    }, {
        'testcase_name': 'pass_and_move',
        'indicator_actions': [[[False, False], [False, False]], [[True, False], [False, False]]],
        'expected_indices': (4, 0)
    }, )
    def test_action_indicators_to_indices_(self, indicator_actions, expected_indices):
        np.testing.assert_array_equal(gojax.action_indicator_to_1d(jnp.array(indicator_actions)), expected_indices)
    ...
    @parameterized.named_parameters({
        ...
        'states': gojax.new_states(board_size=2).at[:, gojax.TURN_CHANNEL_INDEX].set(gojax.WHITES_TURN),
    ...
    def test_action_1d_indices_to_indicator(self):
        board_size = 3
        indicator_actions = gojax.action_1d_to_indicator(jnp.array([5]), nrows=board_size, ncols=board_size)
        np.testing.assert_array_equal(indicator_actions,
                                      [[[False, False, False], [False, False, True], [False, False, False]]])

    def test_action_1d_indices_to_indicator_pass_with_integer(self):
        board_size = 3
        indicator_actions = gojax.action_1d_to_indicator(jnp.array([9]), nrows=board_size, ncols=board_size)
        np.testing.assert_array_equal(indicator_actions,
    ...
    def test_get_killed_channel_index(self):
        ...
        self.assertTrue(jnp.alltrue(
            gojax.get_killed(states) == jnp.array([[[False, False], [False, False]], [[True, True], [True, True]]])))
